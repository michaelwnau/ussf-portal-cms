...
describe('when in preview mode', () => {
  test('returns button with label and default target', async () => {
    global.open = jest.fn()
    ...
    expect(global.open).toHaveBeenCalledWith(value.url, 'ussf_portal_preview')
  })

  test('returns button with label and custom target', async () => {
    global.open = jest.fn()
    const user = userEvent.setup()
    ...
    const value = {
      ...
      label: 'Preview Article',
      isPublished: false,
      target: 'ussf_portal_custom_target_preview',
    }
    const properties = { value: JSON.stringify(value) } as FieldProps<
      typeof controller
    >
    ...
    expect(global.open).toHaveBeenCalledWith(
      value.url,
      'ussf_portal_custom_target_preview'
    )
  })
})
